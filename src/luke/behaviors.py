...
async def run_reflexion(
    ...
    *,
    ...
    prompt = (
        ...
        "     remember(\n"
        f"       id='counterfactual-{goal_id}-<unix_ts>',\n"
        "       type='insight',\n"
        "       title='Counterfactual: <smallest-change-summary>',\n"
        "       content='<full reasoning>',\n"
        f"       tags=['counterfactual', '{goal_id}'],\n"
        f"       links=['reflexion-{goal_id}-<unix_ts>'],\n"
        "       importance=1.3,\n"
        "     )\n\n"
...
async def run_deep_work(bot: Bot, sem: asyncio.Semaphore) -> None:
    ...
    prompt = (
        ...
        "'**Last updated:** <ISO date>', '**Steps completed:** n/m', and a "
        "'## Steps' checklist of 3-7 outcome-named '- [ ]' items\n"
